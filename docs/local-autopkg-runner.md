...
- **`Could not find parent recipe for com.github.<author>.<NameXYZ>`**: AutoPkg failed to resolve a `ParentRecipe` because the `autopkg repo-add` list passed to the runner was incomplete. Munki Manager builds that list from each recipe's stored `trust_info.parent_recipes`, and if the trust chain was *partially* recorded (e.g. when the GitHub PAT used to import the recipe couldn't read part of the chain, or the upstream chain deepened to a third-party author since you last verified — UTM via `flammable-recipes` -> `ahousseini-recipes` is the canonical example), the missing repo never makes it into `run_repo_list.txt`. There are now three layers of defense, in order of when they kick in:
  - **Server-side `verify_trust` walks the live chain.** As of the parent-chain fix, `POST /api/v1/autopkg/recipes/{id}/verify-trust` (and `runs/verify-trust`) compares the **full live chain** to the stored chain instead of just re-hashing stored entries. New ancestors surface as a `pending_approval` `TrustChangeRequest` you must approve in the UI before the next run uses the wider repo list. This is also how trust security is honored end-to-end: any change in any ancestor is visible in the diff and requires explicit approval.
  - **Identifier convention covers the `-recipes`-suffix case.** `_repo_from_identifier` now correctly maps `com.github.ahousseini-recipes.download.UTM` -> `autopkg/ahousseini-recipes` (the segment IS the repo name when it already ends in `-recipes`). Previously it produced `autopkg/ahousseini-recipes-recipes` and silently dropped the repo from `run_repo_list.txt`.
  - **Runner safety net `ensure_parent_repos.py`.** After `autopkg repo-add < run_repo_list.txt`, both the GitHub Actions workflow and `run_local_autopkg.sh` invoke [`AutoPkg/scripts/ensure_parent_repos.py`](../AutoPkg/scripts/ensure_parent_repos.py). It walks every override's on-disk `ParentRecipe` chain **and** non-core `Processor` steps from recipes in those chains, infers the repo for any unresolved ancestor or namespaced processor (e.g. `com.github.grahampugh.recipes.commonprocessors/ChangeModeOwner` → `autopkg/grahampugh-recipes`), and `autopkg repo-add`s it. No-op when everything already resolves; capped at 32 `repo-add` calls per run as a defensive ceiling. This protects existing recipes whose stored trust info pre-dates the chain-walking fix without forcing you to re-verify each one.
- **`Failed local trust verification` on Zoom (verify says OK)**: Zoom's chain is `homebysix.munki.Zoom` → `homebysix.pkg.Zoom` → `hansen-m.download.zoomus` (in `autopkg/hansen-m-recipes`). The download recipe file is `Zoom/Zoom.download.recipe` — the identifier suffix `zoomus` does not match the filename, so server-side trust resolution used to stop at the two homebysix parents. Verify compared that incomplete stored chain to an equally incomplete live walk and reported `verified`. The runner then cloned `hansen-m-recipes` via `ensure_parent_repos`, AutoPkg saw the third ancestor on disk, and local trust failed because `ParentRecipeTrustInfo` lacked `com.github.hansen-m.download.zoomus`. **Fix:** re-verify trust after upgrading (resolution now scans by `.{type}.recipe` and matches the plist `Identifier` field). You should get a `pending_approval` change request adding the `hansen-m` ancestor — approve it before the next run. Stale override entries (e.g. legacy `com.github.hansen-m.download.zoom` or `Zoom7zUnarchiver`) are no longer merged into the runner plist; only canonical DB `trust_info` is sent.

  Recommended fix when you see this error: re-verify trust on the affected recipe (the new `verify_trust` will surface any missing ancestors as `pending_approval`); approve the change request; trigger a new run. The next run also benefits from the two automatic defenses above.
