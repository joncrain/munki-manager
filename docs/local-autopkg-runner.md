...
- **`processor path not found for processor: com.github.<author>.recipes.<ns>/<Name>` / `Failed local trust verification`**: The recipe references a third-party processor whose repo was not cloned and/or whose hash was never recorded in `trust_info.non_core_processors`. Common with recipes like [1Password.munki](https://github.com/autopkg/dataJAR-recipes/blob/master/1Password/1Password.munki.recipe) (Graham Pugh's `ChangeModeOwner`). Fixes are layered the same way as missing parent recipes: re-verify trust on the recipe (the live walk should now resolve namespaced processors into `autopkg/grahampugh-recipes` rather than a non-existent repo named after the full namespace), approve any `pending_approval` change request, and rely on `ensure_parent_repos.py` to `repo-add` the processor repo on the next run if trust info is still stale.
