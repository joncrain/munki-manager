...
# 4. Print the values to set as GitHub repo Variables.
echo
...
These are **Variables**, not Secrets — OIDC federated tokens are scoped to the
exact `subject` claim (`repo:OWNER/REPO:ref:refs/heads/main`), so revealing the
client/tenant/subscription IDs doesn't grant anyone access on its own.
