...
This document describes the **security model** of Munki Manager and the
**known trade-offs** an operator should be aware of before deploying it. It
is not a vulnerability disclosure — it's the design rationale behind the auth
boundaries, the deployment defaults, and the parts of the threat model that
are explicitly an operator's responsibility.

If you're evaluating whether Munki Manager is safe to deploy, read this
together with [`deployment.md`](./deployment.md) and
...
- The Azure storage account uses **managed identity only** (shared-key auth is
  disabled) and the apps assume `Storage Blob Data Contributor` via federated
  identity.
- Two intentional trade-offs you should know about up front:
  1. The Munki package container (`munki-repo`) is **publicly readable by
     default** so Munki clients can fetch pkgs without auth. This matches the
     "unguessable URL" model used by typical CloudFront-fronted Munki
     deployments. Switch to SAS-signed URLs if your installer media is
     sensitive — see
     [§2.3.1](#231-anonymous-read-on-the-munki-repo-container).
  2. The storage account control-plane firewall is **opt-in** via
     `operator_ip_allowlist` — see
     [§2.3.3](#233-no-firewall-on-the-storage-endpoint).
...
Each finding below is labelled with a severity that reflects the **worst case
if the corresponding mitigation is omitted by an operator**. Most are already
mitigated in the default deployment; the label tells you what's at stake if
you turn the mitigation off, not what's wrong with the app today.

- **Critical** — would be exploitable by a remote unauthenticated attacker
  (high impact on Munki client integrity or full data exfiltration) without
  the mitigation.
- **High** — easily exploitable post-credential-leak, or a single
  misconfiguration away from Critical, without the mitigation.
- **Medium** — defense-in-depth gaps or significant blast-radius issues if a
  related secret leaks.
- **Low** — hardening / hygiene; not exploitable on its own.

A finding header marked **✅ Mitigated by default** means the default
configuration already addresses it; the section explains what to watch for if
...
Front Door custom rules) in front, and rotate the signing key. That's the
recommended fix when the contents of the package container are sensitive.
...
`.github/workflows/deploy.yml` uses OIDC + `secrets.AZURE_CLIENT_ID` /
`secrets.AZURE_TENANT_ID` / `secrets.AZURE_SUBSCRIPTION_ID`. This is the right
